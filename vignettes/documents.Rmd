...
The constructor functions in the **pandocfilters** package can also be used to create a new document by reproducing the Abstract Syntax Tree (AST). To show how this is done we use the first paragraph of chapter 2.1 from the [R-FAQ](https://cran.r-project.org/doc/FAQ/R-FAQ.txt)  and apply different inline elements.
...
The new object, `doc` is an R6 object with methods (functions) that allow you to modify the document. To view the available methods, use:
...
Next, append a level 2 header, and then some plain text with formatting:
...
x <- c(
  ...
  "certain system functions, and the ability to run programs ", 
